...
CREATE TABLE IF NOT EXISTS "skc_approval_logs" (
  "id"          UUID        NOT NULL DEFAULT gen_random_uuid(),
  "user_id"     UUID        NOT NULL REFERENCES "skc_users"("id") ON DELETE CASCADE,
  "approved_by" UUID        NOT NULL REFERENCES "skc_users"("id"),
  "action"      "ApprovalLogAction" NOT NULL,
  "note"        TEXT,
  "created_at"  TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT "approval_logs_pkey" PRIMARY KEY ("id")
);
CREATE INDEX IF NOT EXISTS "approval_logs_user_id_idx" ON "skc_approval_logs"("user_id");

-- ---------- 10. skc_fee_config (singleton) ----------
...
-- ---------- 11. skc_student_rating_summary ----------
CREATE TABLE IF NOT EXISTS "skc_student_rating_summary" (
  "id"                     UUID        NOT NULL DEFAULT gen_random_uuid(),
  "student_id"             UUID        NOT NULL REFERENCES "skc_users"("id") ON DELETE CASCADE,
...
CREATE UNIQUE INDEX IF NOT EXISTS "student_rating_summary_student_id_key" ON "skc_student_rating_summary"("student_id");

-- ---------- 12. skc_employer_rating_summary ----------
CREATE TABLE IF NOT EXISTS "skc_employer_rating_summary" (
  "id"                    UUID        NOT NULL DEFAULT gen_random_uuid(),
  "employer_id"           UUID        NOT NULL REFERENCES "skc_users"("id") ON DELETE CASCADE,
  "combined_score"        DOUBLE PRECISION NOT NULL DEFAULT 0,
  "student_review_count"  INTEGER     NOT NULL DEFAULT 0,
  "updated_at"            TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT "employer_rating_summary_pkey" PRIMARY KEY ("id")
);
CREATE UNIQUE INDEX IF NOT EXISTS "employer_rating_summary_employer_id_key" ON "skc_employer_rating_summary"("employer_id");

-- =========================================================================
-- RLS — minimal policies so authenticated skc_users can read/write their rows.
...
ALTER TABLE "skc_approval_logs"              ENABLE ROW LEVEL SECURITY;
ALTER TABLE "skc_fee_config"                 ENABLE ROW LEVEL SECURITY;
ALTER TABLE "skc_student_rating_summary"     ENABLE ROW LEVEL SECURITY;
ALTER TABLE "skc_employer_rating_summary"    ENABLE ROW LEVEL SECURITY;

-- skc_notifications: owner read/update, anyone authenticated can insert (server code inserts on behalf)
...
-- skc_fee_config: everyone authenticated can read; admin/superadmin can update
...
DROP POLICY IF EXISTS "srs_read" ON "skc_student_rating_summary";
CREATE POLICY "srs_read" ON "skc_student_rating_summary" FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "ers_read" ON "skc_employer_rating_summary";
CREATE POLICY "ers_read" ON "skc_employer_rating_summary" FOR SELECT TO authenticated USING (true);
...
-- Agreements / skc_disputes / cancellations / approval logs: let server code manage (coarse policy for pilot)
...
DROP POLICY IF EXISTS "appl_auth_all" ON "skc_approval_logs";
CREATE POLICY "appl_auth_all" ON "skc_approval_logs" FOR ALL TO authenticated USING (true) WITH CHECK (true);
