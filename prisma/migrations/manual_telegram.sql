...
-- 2. Create telegram_link_tokens table (temporary tokens for linking)
...
CREATE INDEX IF NOT EXISTS idx_telegram_tokens_lookup ON telegram_link_tokens(token, used);

-- 3. RLS for telegram_link_tokens
ALTER TABLE telegram_link_tokens ENABLE ROW LEVEL SECURITY;

CREATE POLICY "tlt_insert_self" ON telegram_link_tokens FOR INSERT
  WITH CHECK (user_id = auth.uid());

CREATE POLICY "tlt_select_self" ON telegram_link_tokens FOR SELECT
