-- Manual migration: add job_assignment_requests table
...
CREATE UNIQUE INDEX "job_assignment_requests_job_id_student_id_key"
  ON "job_assignment_requests"("job_id", "student_id");
CREATE INDEX "job_assignment_requests_status_idx"
  ON "job_assignment_requests"("status");
CREATE INDEX "job_assignment_requests_student_id_idx"
  ON "job_assignment_requests"("student_id");

ALTER TABLE "job_assignment_requests"
...
ALTER TABLE "job_assignment_requests" ENABLE ROW LEVEL SECURITY;

-- Students can insert their own request
CREATE POLICY "student_insert_own_request" ON "job_assignment_requests"
  FOR INSERT TO authenticated
  WITH CHECK (student_id = auth.uid());

-- Students can read their own requests
CREATE POLICY "student_read_own_request" ON "job_assignment_requests"
  FOR SELECT TO authenticated
  USING (student_id = auth.uid());

-- Staff/admin can read all and update
CREATE POLICY "staff_read_all_requests" ON "job_assignment_requests"
  FOR SELECT TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM users
      WHERE users.id = auth.uid()
        AND users.role IN ('project_staff','rmutl_staff','admin','superadmin')
    )
  );

CREATE POLICY "staff_update_requests" ON "job_assignment_requests"
