...
export async function POST(request: NextRequest, { params }: { params: Promise<{ jobId: string }> }) {
  ...
  // Get room
  const { data: room } = await supabase.from("skc_job_chat_rooms").select("id, is_locked").eq("job_id", jobId).single();
  ...
  const { data, error } = await supabase.from("skc_chat_messages").insert({
    room_id: room.id,
    sender_id: user.id,
  ...
  }).select("*, sender:skc_users!skc_chat_messages_sender_id_fkey(name, role)").single();
