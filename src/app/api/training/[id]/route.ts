...
export async function GET(
  ...
  const [{ data: course }, { data: modules }, { data: enrollments }] = await Promise.all([
    supabase.from("skc_training_courses")
    ...
    supabase.from("skc_training_enrollments")
      .select("*, trainee:skc_users!skc_training_enrollments_trainee_id_fkey(name, email, role, avatar_url)")
...
export async function PATCH(
  ...
  const { data: course } = await supabase.from("skc_training_courses").select("status, instructor_id").eq("id", id).single();
  if (!course) return NextResponse.json({ error: "ไม่พบหลักสูตร" }, { status: 404 });
  ...
  const { error } = await supabase.from("skc_training_courses").update({ status }).eq("id", id);
